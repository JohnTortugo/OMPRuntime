...
static char driver_name[] = "nexusIO_ddr";

static unsigned long nexusIO_hwaddr_ddr = 0;
static unsigned long nexusIO_length_ddr = 0;

module_param(nexusIO_hwaddr_ddr, ulong, S_IRUSR);
module_param(nexusIO_length_ddr, ulong, S_IRUSR);
...
static int nexusIO_mmap(struct file *filp, struct vm_area_struct *vma)
{
    ...
    if (requested_size != nexusIO_length_ddr)
    {
        printk(KERN_ERR "%s: Error: %lu reserved != %lu requested)\n",
            driver_name, nexusIO_length_ddr, requested_size);
        return -EAGAIN;
    }
    ...
    result = io_remap_pfn_range(vma, vma->vm_start,
        nexusIO_hwaddr_ddr >> PAGE_SHIFT,
        nexusIO_length_ddr, vma->vm_page_prot);
    if (result)
    {
...
static long nexusIO_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        case NEXUS_IOC_HWADDR:
        {
            ...
            if (copy_to_user((void*)arg, &nexusIO_hwaddr_ddr, sizeof(nexusIO_hwaddr_ddr)))
            {
        ...
        case NEXUS_IOC_LENGTH:
        {
            ...
            if (copy_to_user((void*)arg, &nexusIO_length_ddr, sizeof(nexusIO_length_ddr)))
            {
...
static int nexusIO_init(void)
{
    ...
    if (nexusIO_hwaddr_ddr == 0)
    {
        printk(KERN_ERR "%s: No address specified for reserved memory\n", driver_name);
        return -ENODEV;
    }

    if (nexusIO_length_ddr == 0)
    {
        printk(KERN_ERR "%s: No length specified for reserved memory\n", driver_name);
        return -ENODEV;
    }

    printk("%s: nexusIO_hwaddr_ddr=0x%lx, nexusIO_length_ddr=0x%lx\n",
        driver_name, nexusIO_hwaddr_ddr, nexusIO_length_ddr);
