...
void* __mtsp_RuntimeThreadCode(void* params) {
  ...
  mtsp_task_metadata metadata_of_retiring_task;
  ...
  while (true) {
    ...
    if (RetirementQueue.try_deq(&task)) {
      //TODO: Make this faster by only copying the globalTaskId and
      //      storing the end values of other fields directly to the
      //      taskMetadata array
      metadata_of_retiring_task = *(task->metadata);
      removeFromTaskGraph(task);

#ifdef MEASURE_RETIREMENT
      kmp_uint64 end = end_read_mtsp();
      metadata_of_retiring_task.cycles_retirement += (end - start);
#endif

#ifdef MEASURE_TASK_SIZE
      if (metadata_of_retiring_task.globalTaskId >= MAXIMUM_EXPECTED_TASKS) {
